const ServiceData = {
    pageH1: 'Title',
    pageH2: 'Subtext',
    description:
        'Lorem ipsum dolor sit amet consectetur, adipisicing elit. Optio doloribus quo explicabo soluta distinctio, odiodeserunt eius deleniti adipisci asperiores enim. Qui quam,similique assumenda dignissimos corporis repellatperspiciatis quod odio laboriosam officiis temporibus dolores omnis debitis. Nihil itaque reiciendis voluptate beatae nemo quaerat animi impedit. Natus dicta rem odit, aliquid reprehenderit non nesciunt praesentium repellendusexercitationem. Amet labore distinctio minus reiciendis officia eius et laudantium. Voluptas iure officiis quidem commodi minus dolor illo placeat.',
    offerings: [
        {
            icon: 'https://dummyimage.com/150x150/000/fff',
            title: 'Title 1',
            description:
                'Lorem ipsum dolor sit amet consectetur, adipisicing elit.Optio doloribus quo explicabo soluta distinctio, odiodeserunt eius deleniti adipisci asperiores enim. Qui quam,similique assumenda dignissimos corporis repellatperspiciatis quod odio laboriosam officiis temporibus',
            bullets: [
                'Accounts Payable',
                'Detail 2',
                'Detail 3',
                'Detail 4',
                'Detail 5',
                'Detail 6',
                'Detail 7',
                'Detail 8'
            ]
        },
        {
            icon: 'https://dummyimage.com/150x150/000/fff',
            title: 'Title 2',
            description:
                'Lorem ipsum dolor sit amet consectetur, adipisicing elit.Optio doloribus quo explicabo soluta distinctio, odiodeserunt eius deleniti adipisci asperiores enim. Qui quam,similique assumenda dignissimos corporis repellatperspiciatis quod odio laboriosam officiis temporibus',
            bullets: [
                'Detail 1',
                'Detail 2',
                'Detail 3',
                'Detail 4',
                'Detail 5',
                'Detail 6'
            ]
        },
        {
            icon: 'https://dummyimage.com/150x150/000/fff',
            title: 'Title 1',
            description:
                'Lorem ipsum dolor sit amet consectetur, adipisicing elit.Optio doloribus quo explicabo soluta distinctio, odiodeserunt eius deleniti adipisci asperiores enim. Qui quam,similique assumenda dignissimos corporis repellatperspiciatis quod odio laboriosam officiis temporibus',
            bullets: [
                'Detail 1',
                'Detail 2',
                'Detail 3',
                'Detsadfasail 4',
                'Detail 5',
                'Detasadfasdfsdzfafil 6',
                'Detail 7'
            ]
        }
    ]
}

export default ServiceData
